...
import com.XSDrakor.XSDrakor.Companion.cinemaOSApi
import com.XSDrakor.XSDrakor.Companion.Player4uApi
import com.XSDrakor.XSDrakor.Companion.idlixAPI
import com.XSDrakor.XSDrakor.Companion.RiveStreamAPI
...
object XSDrakorExtractor : XSDrakor() {
    ...
    suspend fun invokeGomovies(
        ...
        invokeGpress(title, year, season, episode, callback, XSDrakor.gomoviesAPI, "Gomovies", base64Decode("X3NtUWFtQlFzRVRi"), base64Decode("X3NCV2NxYlRCTWFU"))
    }
    ...
    suspend fun invokeVidsrccc(
        ...
        val url = if (season == null) "${XSDrakor.vidsrcccAPI}/v2/embed/movie/$tmdbId" else "${XSDrakor.vidsrcccAPI}/v2/embed/tv/$tmdbId/$season/$episode"
        ...
        val serverUrl = if (season == null) "${XSDrakor.vidsrcccAPI}/api/$tmdbId/servers?id=$tmdbId&type=movie&v=$v&vrf=$vrf&imdbId=$imdbId" else "${XSDrakor.vidsrcccAPI}/api/$tmdbId/servers?id=$tmdbId&type=tv&v=$v&vrf=$vrf&imdbId=$imdbId&season=$season&episode=$episode"
        app.get(serverUrl).parsedSafe<VidsrcccResponse>()?.data?.amap {
            val sources = app.get("${XSDrakor.vidsrcccAPI}/api/source/${it.hash}").parsedSafe<VidsrcccResult>()?.data ?: return@amap
            when {
                it.name.equals("VidPlay") -> {
                    callback.invoke(newExtractorLink("VidPlay", "VidPlay", sources.source ?: return@amap, ExtractorLinkType.M3U8) { this.referer = "${XSDrakor.vidsrcccAPI}/" })
                ...
                it.name.equals("UpCloud") -> {
                    val scriptData = app.get(sources.source ?: return@amap, referer = "${XSDrakor.vidsrcccAPI}/").document.selectFirst("script:containsData(source =)")?.data()
                    ...
                    app.get("${iframe.substringBeforeLast("/")}/getSources?id=$id&_k=$key", headers = mapOf("X-Requested-With" to "XMLHttpRequest"), referer = iframe).parsedSafe<UpcloudResult>()?.sources?.amap file@{ source ->
                        callback.invoke(newExtractorLink("UpCloud", "UpCloud", source.file ?: return@file, ExtractorLinkType.M3U8) { this.referer = "${XSDrakor.vidsrcccAPI}/" })
                    }
                }
            }
        }
    ...
    suspend fun invokeVidsrc(
        ...
        val url = if (season == null) "${XSDrakor.vidSrcAPI}/embed/movie?imdb=$imdbId" else "${XSDrakor.vidSrcAPI}/embed/tv?imdb=$imdbId&season=$season&episode=$episode"
    ...
    suspend fun invokeXprime(
        ...
        runAllAsync(
            {
                val url = if (season == null) "${XSDrakor.xprimeAPI}/${servers.first()}?id=$tmdbId" else "${XSDrakor.xprimeAPI}/${servers.first()}?id=$tmdbId&season=$season&episode=$episode"
            ...
            {
                val url = if (season == null) "${XSDrakor.xprimeAPI}/${servers.last()}?name=$title&fallback_year=$year" else "${XSDrakor.xprimeAPI}/${servers.last()}?name=$title&fallback_year=$year&season=$season&episode=$episode"
    ...
    suspend fun invokeWatchsomuch(
        ...
        val epsId = app.post("${XSDrakor.watchSomuchAPI}/Watch/ajMovieTorrents.aspx", data = mapOf("index" to "0", "mid" to "$id", "wsk" to "30fb68aa-1c71-4b8c-b5d4-4ca9222cfb45", "lid" to "", "liu" to ""), headers = mapOf("X-Requested-With" to "XMLHttpRequest")).parsedSafe<WatchsomuchResponses>()?.movie?.torrents?.let { eps -> if (season == null) eps.firstOrNull()?.id else eps.find { it.episode == episode && it.season == season }?.id } ?: return
        val (seasonSlug, episodeSlug) = getEpisodeSlug(season, episode)
        val subUrl = if (season == null) "${XSDrakor.watchSomuchAPI}/Watch/ajMovieSubtitles.aspx?mid=$id&tid=$epsId&part=" else "${XSDrakor.watchSomuchAPI}/Watch/ajMovieSubtitles.aspx?mid=$id&tid=$epsId&part=S${seasonSlug}E${episodeSlug}"
        app.get(subUrl).parsedSafe<WatchsomuchSubResponses>()?.subtitles?.map { sub -> subtitleCallback.invoke(newSubtitleFile(sub.label?.substringBefore("&nbsp")?.trim() ?: "", fixUrl(sub.url ?: return@map null, XSDrakor.watchSomuchAPI))) }
    }
    ...
    suspend fun invokeMapple(
        ...
        val url = if (season == null) "${XSDrakor.mappleAPI}/watch/$mediaType/$tmdbId" else "${XSDrakor.mappleAPI}/watch/$mediaType/$season-$episode/$tmdbId"
        ...
        callback.invoke(newExtractorLink("Mapple", "Mapple", videoLink ?: return, ExtractorLinkType.M3U8) { this.referer = "${XSDrakor.mappleAPI}/"; this.headers = mapOf("Accept" to "*/*") })
        val subRes = app.get("${XSDrakor.mappleAPI}/api/subtitles?id=$tmdbId&mediaType=$mediaType${if (season == null) "" else "&season=1&episode=1"}", referer = "${XSDrakor.mappleAPI}/").text
        tryParseJson<ArrayList<MappleSubtitle>>(subRes)?.map { subtitle -> subtitleCallback.invoke(newSubtitleFile(subtitle.display ?: "", fixUrl(subtitle.url ?: return@map, XSDrakor.mappleAPI))) }
    }
    ...
    suspend fun invokeVidlink(
        ...
        val url = if (season == null) "${XSDrakor.vidlinkAPI}/$type/$tmdbId" else "${XSDrakor.vidlinkAPI}/$type/$tmdbId/$season/$episode"
        val videoLink = app.get(url, interceptor = WebViewResolver(Regex("""${XSDrakor.vidlinkAPI}/api/b/$type/A{32}"""), timeout = 15_000L)).parsedSafe<VidlinkSources>()?.stream?.playlist
        callback.invoke(newExtractorLink("Vidlink", "Vidlink", videoLink ?: return, ExtractorLinkType.M3U8) { this.referer = "${XSDrakor.vidlinkAPI}/" })
    }
    ...
    suspend fun invokeVidfast(
        ...
        val type = if (season == null) "movie" else "tv"
        val url = if (season == null) "${XSDrakor.vidfastAPI}/$type/$tmdbId" else "${XSDrakor.vidfastAPI}/$type/$tmdbId/$season/$episode"
        val res = app.get(url, interceptor = WebViewResolver(Regex("""${XSDrakor.vidfastAPI}/$module/JEwECseLZdY"""), timeout = 15_000L)).text
        tryParseJson<ArrayList<VidFastServers>>(res)?.filter { it.description?.contains("Original audio") == true }?.amapIndexed { index, server ->
                val source = app.get("${XSDrakor.vidfastAPI}/$module/Sdoi/${server.data}", referer = "${XSDrakor.vidfastAPI}/").parsedSafe<VidFastSources>()
    ...
    suspend fun invokeWyzie(
        ...
        val url = if (season == null) "${XSDrakor.wyzieAPI}/search?id=$tmdbId" else "${XSDrakor.wyzieAPI}/search?id=$tmdbId&season=$season&episode=$episode"
    ...
    suspend fun invokeVixsrc(
        ...
        val type = if (season == null) "movie" else "tv"
        val url = if (season == null) "${XSDrakor.vixsrcAPI}/$type/$tmdbId" else "${XSDrakor.vixsrcAPI}/$type/$tmdbId/$season/$episode"
        ...
        listOf(VixsrcSource("Vixsrc [Alpha]", video1, url), VixsrcSource("Vixsrc [Beta]", video2, "${XSDrakor.mappleAPI}/")).map {
    ...
    suspend fun invokeSuperembed(
        ...
        val token = app.get("${XSDrakor.superembedAPI}/directstream.php?video_id=$tmdbId&tmdb=1$path").url.substringAfter("?play=")
    ...
    suspend fun invokeVidrock(
        ...
        val type = if (season == null) "movie" else "tv"
        val url = "${XSDrakor.vidrockAPI}/$type/$tmdbId${if (type == "movie") "" else "/$season/$episode"}"
        val encryptData = VidrockHelper.encrypt(tmdbId, type, season, episode)
        app.get("${XSDrakor.vidrockAPI}/api/$type/$encryptData", referer = url).parsedSafe<LinkedHashMap<String, HashMap<String, String>>>()?.map { source ->
            if (source.key == "source2") {
                val json = app.get(source.value["url"] ?: return@map, referer = "${XSDrakor.vidrockAPI}/").text
                tryParseJson<ArrayList<VidrockSource>>(json)?.reversed()?.map mirror@{ it ->
                    callback.invoke(newExtractorLink("Vidrock", "Vidrock [Source2]", it.url ?: return@mirror, INFER_TYPE) { this.quality = it.resolution ?: Qualities.Unknown.value; this.headers = mapOf("Range" to "bytes=0-", "Referer" to "${XSDrakor.vidrockAPI}/") })
                }
            } else {
                callback.invoke(newExtractorLink("Vidrock", "Vidrock [${source.key.capitalize()}]", source.value["url"] ?: return@map, ExtractorLinkType.M3U8) { this.referer = "${XSDrakor.vidrockAPI}/"; this.headers = mapOf("Origin" to XSDrakor.vidrockAPI) })
            }
        }
