...
import com.XSCinemax21.XSCinemax21.Companion.cinemaOSApi
import com.XSCinemax21.XSCinemax21.Companion.Player4uApi
import com.XSCinemax21.XSCinemax21.Companion.idlixAPI
import com.XSCinemax21.XSCinemax21.Companion.RiveStreamAPI
...
object XSCinemax21Extractor : XSCinemax21() {
    ...
    suspend fun invokeGomovies(
        ...
        invokeGpress(title, year, season, episode, callback, XSCinemax21.gomoviesAPI, "Gomovies", base64Decode("X3NtUWFtQlFzRVRi"), base64Decode("X3NCV2NxYlRCTWFU"))
    }
    ...
    suspend fun invokeVidsrccc(
        ...
        val url = if (season == null) "${XSCinemax21.vidsrcccAPI}/v2/embed/movie/$tmdbId" else "${XSCinemax21.vidsrcccAPI}/v2/embed/tv/$tmdbId/$season/$episode"
        ...
        val serverUrl = if (season == null) "${XSCinemax21.vidsrcccAPI}/api/$tmdbId/servers?id=$tmdbId&type=movie&v=$v&vrf=$vrf&imdbId=$imdbId" else "${XSCinemax21.vidsrcccAPI}/api/$tmdbId/servers?id=$tmdbId&type=tv&v=$v&vrf=$vrf&imdbId=$imdbId&season=$season&episode=$episode"
        app.get(serverUrl).parsedSafe<VidsrcccResponse>()?.data?.amap {
            val sources = app.get("${XSCinemax21.vidsrcccAPI}/api/source/${it.hash}").parsedSafe<VidsrcccResult>()?.data ?: return@amap
            when {
                it.name.equals("VidPlay") -> {
                    callback.invoke(newExtractorLink("VidPlay", "VidPlay", sources.source ?: return@amap, ExtractorLinkType.M3U8) { this.referer = "${XSCinemax21.vidsrcccAPI}/" })
                ...
                it.name.equals("UpCloud") -> {
                    val scriptData = app.get(sources.source ?: return@amap, referer = "${XSCinemax21.vidsrcccAPI}/").document.selectFirst("script:containsData(source =)")?.data()
                    ...
                    app.get("${iframe.substringBeforeLast("/")}/getSources?id=$id&_k=$key", headers = mapOf("X-Requested-With" to "XMLHttpRequest"), referer = iframe).parsedSafe<UpcloudResult>()?.sources?.amap file@{ source ->
                        callback.invoke(newExtractorLink("UpCloud", "UpCloud", source.file ?: return@file, ExtractorLinkType.M3U8) { this.referer = "${XSCinemax21.vidsrcccAPI}/" })
                    }
                }
            }
        }
    ...
    suspend fun invokeVidsrc(
        ...
        val url = if (season == null) "${XSCinemax21.vidSrcAPI}/embed/movie?imdb=$imdbId" else "${XSCinemax21.vidSrcAPI}/embed/tv?imdb=$imdbId&season=$season&episode=$episode"
    ...
    suspend fun invokeXprime(
        ...
        runAllAsync(
            {
                val url = if (season == null) "${XSCinemax21.xprimeAPI}/${servers.first()}?id=$tmdbId" else "${XSCinemax21.xprimeAPI}/${servers.first()}?id=$tmdbId&season=$season&episode=$episode"
            ...
            {
                val url = if (season == null) "${XSCinemax21.xprimeAPI}/${servers.last()}?name=$title&fallback_year=$year" else "${XSCinemax21.xprimeAPI}/${servers.last()}?name=$title&fallback_year=$year&season=$season&episode=$episode"
    ...
    suspend fun invokeWatchsomuch(
        ...
        val epsId = app.post("${XSCinemax21.watchSomuchAPI}/Watch/ajMovieTorrents.aspx", data = mapOf("index" to "0", "mid" to "$id", "wsk" to "30fb68aa-1c71-4b8c-b5d4-4ca9222cfb45", "lid" to "", "liu" to ""), headers = mapOf("X-Requested-With" to "XMLHttpRequest")).parsedSafe<WatchsomuchResponses>()?.movie?.torrents?.let { eps -> if (season == null) eps.firstOrNull()?.id else eps.find { it.episode == episode && it.season == season }?.id } ?: return
        val (seasonSlug, episodeSlug) = getEpisodeSlug(season, episode)
        val subUrl = if (season == null) "${XSCinemax21.watchSomuchAPI}/Watch/ajMovieSubtitles.aspx?mid=$id&tid=$epsId&part=" else "${XSCinemax21.watchSomuchAPI}/Watch/ajMovieSubtitles.aspx?mid=$id&tid=$epsId&part=S${seasonSlug}E${episodeSlug}"
        app.get(subUrl).parsedSafe<WatchsomuchSubResponses>()?.subtitles?.map { sub -> subtitleCallback.invoke(newSubtitleFile(sub.label?.substringBefore("&nbsp")?.trim() ?: "", fixUrl(sub.url ?: return@map null, XSCinemax21.watchSomuchAPI))) }
    }
    ...
    suspend fun invokeMapple(
        ...
        val url = if (season == null) "${XSCinemax21.mappleAPI}/watch/$mediaType/$tmdbId" else "${XSCinemax21.mappleAPI}/watch/$mediaType/$season-$episode/$tmdbId"
        ...
        callback.invoke(newExtractorLink("Mapple", "Mapple", videoLink ?: return, ExtractorLinkType.M3U8) { this.referer = "${XSCinemax21.mappleAPI}/"; this.headers = mapOf("Accept" to "*/*") })
        val subRes = app.get("${XSCinemax21.mappleAPI}/api/subtitles?id=$tmdbId&mediaType=$mediaType${if (season == null) "" else "&season=1&episode=1"}", referer = "${XSCinemax21.mappleAPI}/").text
        tryParseJson<ArrayList<MappleSubtitle>>(subRes)?.map { subtitle -> subtitleCallback.invoke(newSubtitleFile(subtitle.display ?: "", fixUrl(subtitle.url ?: return@map, XSCinemax21.mappleAPI))) }
    }
    // ================== VIDLINK SOURCE ==================
    suspend fun invokeVidlink(
        ...
        val url = if (season == null) "${XSCinemax21.vidlinkAPI}/$type/$tmdbId" else "${XSCinemax21.vidlinkAPI}/$type/$tmdbId/$season/$episode"
        val videoLink = app.get(url, interceptor = WebViewResolver(Regex("""${XSCinemax21.vidlinkAPI}/api/b/$type/A{32}"""), timeout = 15_000L)).parsedSafe<VidlinkSources>()?.stream?.playlist
        callback.invoke(newExtractorLink("Vidlink", "Vidlink", videoLink ?: return, ExtractorLinkType.M3U8) { this.referer = "${XSCinemax21.vidlinkAPI}/" })
    }
    ...
    suspend fun invokeVidfast(
        ...
        val type = if (season == null) "movie" else "tv"
        val url = if (season == null) "${XSCinemax21.vidfastAPI}/$type/$tmdbId" else "${XSCinemax21.vidfastAPI}/$type/$tmdbId/$season/$episode"
        val res = app.get(url, interceptor = WebViewResolver(Regex("""${XSCinemax21.vidfastAPI}/$module/JEwECseLZdY"""), timeout = 15_000L)).text
        tryParseJson<ArrayList<VidFastServers>>(res)?.filter { it.description?.contains("Original audio") == true }?.amapIndexed { index, server ->
                val source = app.get("${XSCinemax21.vidfastAPI}/$module/Sdoi/${server.data}", referer = "${XSCinemax21.vidfastAPI}/").parsedSafe<VidFastSources>()
    ...
    suspend fun invokeWyzie(
        ...
        val url = if (season == null) "${XSCinemax21.wyzieAPI}/search?id=$tmdbId" else "${XSCinemax21.wyzieAPI}/search?id=$tmdbId&season=$season&episode=$episode"
    ...
    suspend fun invokeVixsrc(
        ...
        val type = if (season == null) "movie" else "tv"
        val url = if (season == null) "${XSCinemax21.vixsrcAPI}/$type/$tmdbId" else "${XSCinemax21.vixsrcAPI}/$type/$tmdbId/$season/$episode"
        ...
        listOf(VixsrcSource("Vixsrc [Alpha]", video1, url), VixsrcSource("Vixsrc [Beta]", video2, "${XSCinemax21.mappleAPI}/")).map {
    ...
    suspend fun invokeSuperembed(
        ...
        val token = app.get("${XSCinemax21.superembedAPI}/directstream.php?video_id=$tmdbId&tmdb=1$path").url.substringAfter("?play=")
    ...
    suspend fun invokeVidrock(
        ...
        val type = if (season == null) "movie" else "tv"
        val url = "${XSCinemax21.vidrockAPI}/$type/$tmdbId${if (type == "movie") "" else "/$season/$episode"}"
        val encryptData = VidrockHelper.encrypt(tmdbId, type, season, episode)
        app.get("${XSCinemax21.vidrockAPI}/api/$type/$encryptData", referer = url).parsedSafe<LinkedHashMap<String, HashMap<String, String>>>()?.map { source ->
            if (source.key == "source2") {
                val json = app.get(source.value["url"] ?: return@map, referer = "${XSCinemax21.vidrockAPI}/").text
                tryParseJson<ArrayList<VidrockSource>>(json)?.reversed()?.map mirror@{ it ->
                    callback.invoke(newExtractorLink("Vidrock", "Vidrock [Source2]", it.url ?: return@mirror, INFER_TYPE) { this.quality = it.resolution ?: Qualities.Unknown.value; this.headers = mapOf("Range" to "bytes=0-", "Referer" to "${XSCinemax21.vidrockAPI}/") })
                }
            } else {
                callback.invoke(newExtractorLink("Vidrock", "Vidrock [${source.key.capitalize()}]", source.value["url"] ?: return@map, ExtractorLinkType.M3U8) { this.referer = "${XSCinemax21.vidrockAPI}/"; this.headers = mapOf("Origin" to XSCinemax21.vidrockAPI) })
            }
        }
